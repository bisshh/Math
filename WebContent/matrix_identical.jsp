...
String result;
int addM, row, col, flag;

if(request.getParameter("submit")!=null)
{
	addM = Integer.valueOf(request.getParameter("addM"));
	row = Integer.valueOf(request.getParameter("row"));
	col = Integer.valueOf(request.getParameter("col"));
	flag = 0;
	
    if(!Array.isArray(addM)) {
        result = "Please Create Matrix First by Clicking on Create Matrix!";
        out.print (result);
    }
    for(int c = 0; c < row; c++){
        for(int d = 0; d < col; d++){
						                ...
						                function createMatrix() {
						                    ...
						                    if(row!=col){
						                        ...
						                        for (c = 0; c < row; c++) {
						                            ...
						                            for (d = 0; d < col; d++) {
						                                htmlA += '<td>';
						                                htmlA += '<input type="text" name="addM[' + row + '][' + col + ']">';
						                                htmlA += '</td>';
						                            }
						                            htmlA += '</tr>';
						                        }
						                        htmlA += '</table>';
						                        document.getElementById('addM').innerHTML = htmlA;
